...
sequenceDiagram
    ...
    participant Driver as OPCUAConnection (asyncua)
    participant PLC as Physical PLC

    App->>Driver: connect()
    Driver->>PLC: TCP Connect to opc.tcp://IP:4840
    PLC-->>Driver: OPC-UA Session Established
    Driver->>Driver: Initialize Node Handle Cache (_plc_node_cache)
    
    loop Every 100ms Telemetry Poll
        App->>Driver: get_node("ns=4, s=System_State")
        Driver->>PLC: Read Value (binary opc.tcp)
        PLC-->>Driver: Return Variant Value
        Driver-->>App: Return Python Primitive
    end

    opt Write Command (e.g. BEARING_ON)
        App->>Driver: set_node_state("ns=4, s=BEARING_ON", True)
        Driver->>PLC: Write Value (ns=4, s=BEARING_ON = True)
        PLC-->>Driver: Write Response OK
    end

    opt Health Check Failure (Socket Drop)
        Driver->>Driver: Health Check Fails (every 5s)
        Driver->>Driver: Disconnect & Clear Cache
        Driver->>PLC: Auto-Reconnect Attempt
    end
```
