                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "10px", textAlign: "left", fontFamily: "JetBrains Mono" }}>
                  ...
                  <tbody>
                    ...
                    {activeModal === "energy" && (
                      <>
                        <tr style={{ borderBottom: "1px solid #323842" }}>
                          <td style={{ padding: "8px 12px", color: "#8f9097" }}>{getRegRange(b3, 0)}</td>
                          <td style={{ padding: "8px 12px", color: "#e1e2ea" }}>Active Power</td>
                          <td style={{ padding: "8px 12px", color: "#e1e2ea" }}>{data?.raw?.vibit3?.raw_power_regs ? data.raw.vibit3.raw_power_regs.join(", ") : "---"}</td>
                          <td style={{ padding: "8px 12px", color: "#8f9097" }}>{data?.raw?.vibit3?.raw_power_regs ? data.raw.vibit3.raw_power_regs.map(r => "0x" + r.toString(16).toUpperCase()).join(", ") : "---"}</td>
                        </tr>
                        <tr>
                          <td style={{ padding: "8px 12px", color: "#8f9097" }}>{getRegRange(b3, 100)}</td>
                          <td style={{ padding: "8px 12px", color: "#e1e2ea" }}>Accumulated Energy</td>
                          <td style={{ padding: "8px 12px", color: "#e1e2ea" }}>{data?.raw?.vibit3?.raw_kwh_regs ? data.raw.vibit3.raw_kwh_regs.join(", ") : "---"}</td>
                          <td style={{ padding: "8px 12px", color: "#8f9097" }}>{data?.raw?.vibit3?.raw_kwh_regs ? data.raw.vibit3.raw_kwh_regs.map(r => "0x" + r.toString(16).toUpperCase()).join(", ") : "---"}</td>
                        </tr>
                      </>
                    )}
                    {activeModal === "axis" && (
