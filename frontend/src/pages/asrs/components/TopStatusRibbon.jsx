...
const TopStatusRibbon = ({ plcConnected, ledConnected, shuttleState }) => {
  ...
  const getStatusConfig = (type, val) => {
    ...
    if (val === 'moving' || val === 'busy') {
      return {
        label: `SHUTTLE: ${val.toUpperCase()}${shuttlePosition ? ` @ ${shuttlePosition}` : ''}`,
    ...
    return {
      label: `SHUTTLE: IDLE${shuttlePosition ? ` @ ${shuttlePosition}` : ''}`,
  ...
  const feedConfig = getStatusConfig('feed', ledStatus);
  const shuttleConfig = getStatusConfig('shuttle', shuttleStatus);
  ...
  return (
    ...
    >
      ...
      {renderBadge(shuttleConfig)}
    </div>
  );
};

export default TopStatusRibbon;
