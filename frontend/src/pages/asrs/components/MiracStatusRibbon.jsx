...
const MiracStatusRibbon = ({ plcConnected, wsStatus, spindleSpeed, cycleStart }) => {
  ...
  const getStatusConfig = (type, val) => {
    ...
    // CNC Lathe Spindle configurations
    if (val === 'running') {
      return {
  ...
  const latheConfig = getStatusConfig('lathe', miracState);
  ...
  return (
    ...
    >
      ...
      {renderBadge(latheConfig)}
    </div>
  );
};

export default MiracStatusRibbon;
