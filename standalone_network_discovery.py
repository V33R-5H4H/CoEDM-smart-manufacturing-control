...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║              Standalone Network Discovery & Port Scanner                    ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Scans a network subnet for:                                                 ║
║    • Live hosts (ICMP ping sweep)                                          ║
║    • Open industrial ports (OPC-UA, Modbus, HTTP, SSH, MQTT …)             ║
║    • Protocol identification on open ports                                 ║
║                                                                            ║
║  Usage:                                                                    ║
║    python standalone_network_discovery.py                                  ║
║    python standalone_network_discovery.py --subnet 192.168.1.0/24          ║
║    python standalone_network_discovery.py --range 100-130                  ║
║    python standalone_network_discovery.py --json                           ║
║    python standalone_network_discovery.py --ports 502,4840,80              ║
║                                                                            ║
║  Requirements: Only standard Python libraries (no external dependencies)!  ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
# Default known devices (you can customize this list!)
KNOWN_DEVICES = {
    # "10.10.14.100": {
    #     "name": "Example PLC",
    #     "protocol": "OPC-UA",
...
# Industrial port definitions (standard mappings)
INDUSTRIAL_PORTS = {
    20:    "FTP (data)",
    21:    "FTP (control)",
    22:    "SSH",
...
# Quick-scan subset for fast default scan
...
def network_scan(
    ...
    if not json_output:
        print(f"\n{_BOLD}{_BAR}{_RESET}")
        print(f"{_BOLD}  Standalone Network Discovery & Port Scanner{_RESET}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Standalone Network Discovery — scan a network subnet",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python standalone_network_discovery.py                     Quick scan (default subnet 10.10.14.0/24)
  python standalone_network_discovery.py --subnet 192.168.1.0/24    Specify subnet
  python standalone_network_discovery.py --range 100-130           Scan .100 to .130 only
  python standalone_network_discovery.py --full                  Scan ALL standard industrial ports
  python standalone_network_discovery.py --ports 502,4840,80       Custom port list
  python standalone_network_discovery.py --json                    Machine-readable JSON output
  python standalone_network_discovery.py --json > scan.json        Save to file
  python standalone_network_discovery.py --timeout 2               Slower network timeout
  python standalone_network_discovery.py --no-identify             Skip protocol identification
        """,
    )
    parser.add_argument(
        "--subnet",
